...
BALL_RADIUS = 10
POCKET_RADIUS = 18
FRICTION = 0.975   # stronger friction (slower balls)
MIN_VEL = 0.05
BORDER = 30
...
pygame.display.set_caption("Immersive Pool (Improved)")
...
class Ball:
    ...
    def move(self):
        ...
        # velocity cap (prevents erratic motion)
...
def collide(b1,b2):
    ...
    if dist < BALL_RADIUS*2:
        ...
        # softer collision (less chaotic)
        impulse = -1.1 * vel_along
...
def check_game_over(balls):
    remaining = [b for b in balls if b.active and not b.is_cue]
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if game.state == "MENU":
            ...
            if event.type == pygame.MOUSEBUTTONUP and game.dragging:
                ...
                if vec.length() > 0:
                    cue.vel = vec.normalize() * (game.power * 0.15)  # slower shots
                    game.state = "MOVING"
                    game.dragging = False
                    game.power = 0
    ...
    if game.state == "MENU":
        ...
        screen.blit(sub,(WIDTH//2-110, HEIGHT//2+20))
