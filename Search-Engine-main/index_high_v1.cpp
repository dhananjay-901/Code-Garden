//multi worded search with stop words and snippets
...
string getSnippet(const string& content, const string& query) {
    ...
    // simple highlight (uppercase)
    for (auto& w : tokenize(query)) {
    ...
    //return "..." + content.substr(start, end - start) + "..."; for no highlighting
}
